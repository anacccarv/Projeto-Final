...
tomate |>
  ...
  geom_point(size = 1.5) +
  geom_errorbar(aes(ymin = sev_mean - sev_sd,
                    ymax = sev_mean + sev_sd),
                width = 0.1) +
  theme_classic() +
  labs( x = "Tempo",
        y = "Severidade (%)",
...
Os dados foram filtrados para incluir apenas informações do dia 28, considerando o rendimento final dos tomates.
...
Para um resumo visual do rendimento médio por produto:
...
tomate2 |>
  ...
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = yld_mean - yld_sd,
                    ymax = yld_mean + yld_sd),
                width = 0.1) +
  theme_classic() +
  labs( x = "Produto",
       y = "Rendimento (%)",
...
Observamos uma variação no rendimento entre os diferentes produtos testados (Check, Bio agent B, Fungicide, Bio agent A). O produto Fungicide parece apresentar um rendimento médio mais alto em comparação aos demais, enquanto Check mostrou o menor rendimento médio, como esperado. Esses resultados fornecem insights iniciais sobre o desempenho dos produtos testados, mas análises mais detalhadas, incluindo testes estatísticos específicos para comparações entre os produtos, podem oferecer uma compreensão mais robusta das diferenças observadas.
